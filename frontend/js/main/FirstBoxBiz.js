...
export default class FirstBoxBiz {
    ...
    static chkRoomName(roomName) {
        if (StringUtil.chkObjNull(roomName)) {
            MessageBoxControl.open("Room Title can't be empty!");
            return false;
        }
        return true;
    }
    static chkMaxNumber(maxNumber) {
        if (StringUtil.chkObjNull(maxNumber)) {
            MessageBoxControl.open("Room Maximum Number can't be empty!");
            return false;
        }
        let numV = parseInt(maxNumber);
        if (window.isNaN(numV)) {
            MessageBoxControl.open("Room Maximum Number must be a number!");
            return false;
        }
        if (numV < 2) {
            MessageBoxControl.open("Room Maximum Number must be greater than 1!");
            return false;
        }
        return true;
    }
