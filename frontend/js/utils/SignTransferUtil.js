...
class SignTransferUtil {
    ...
    static sendAppointMessage(message, key) {
        if (!this.sendMessageStateMap.get(key)) {
            MessageBoxControl.open("Message Info Channel is not opened or an error has occurred and the channel has been closed!");
            return;
        }
    ...
    static sendAllMessage(message) {
        ...
        if (!isAnyone) {
            MessageBoxControl.open("No one has entered or the message channel has not been opened or an error has occurred and the channel has been closed!");
        }
    }
    ...
    static handleChannel(key, dc) {
        ...
        dc.onerror = (event) => {
            ...
            MessageBoxControl.open("Error occurred in the message channel: " + event.error.message + "，message channel has been closed！", () => window.location.reload());
        };
    }
}
_a = SignTransferUtil;
