...
export default class ChatMessageControl {
    constructor() {
        ...
        chatMessageHeaderBox.innerText = 'The ID corresponds to the person\'s name, which corresponds to the id above the video display box.';
        ...
        this.chatMessageSendBtnEl.innerText = 'Send';
    ...
    createReceiveFile(id, fileName, url) {
        ...
        if (StringUtil.chkObjNull(suffixName)) {
            chatMessageBodyMessageFileStyleText.innerText = 'Unknown file';
        }
        else {
            chatMessageBodyMessageFileStyleText.innerText = suffixName + 'Document';
        }
        ...
        chatMessageBodyMessageFileDownloadBtn.innerText = 'Download';
    ...
    bindSendMessageFun(id, callBackFun) {
        this.chatMessageSendBtnEl.addEventListener('click', () => {
            ...
            if (message.length > FileUtil.maxMessageLenght) {
                MessageBoxControl.open("Your message is too long!");
    ...
    bindSelectSendFileFun(id, callBackFun) {
        ...
        this.hideFileInputEl.addEventListener('change', () => {
            ...
            if (file.size > FileUtil.fileMaxSize) {
                MessageBoxControl.open("File size is too large!" + Math.floor(FileUtil.fileMaxSize / 1024 / 1024) + "MB!");
                return;
            }
